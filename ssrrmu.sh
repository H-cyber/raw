...
ssr_link_qr(){
	...
	Param_base64_username=$(urlsafe_base64 "${user_name}")
	SSRbase64=$(urlsafe_base64 "${ip}:${port}:${SSRprotocol}:${method}:${SSRobfs}:${SSRPWDbase64}/?obfsparam=&protoparam=&remarks=${Param_base64_username}&group=")
...
View_User_info(){
	...
	echo -e "${ss_link}"
	echo -e "${ssr_link}"
	echo "${SSRurl}" > ~/ssr.conf
        echo "${SSurl}" > ~/ss.conf
        echo -e "SSR QR CODE: "
        echo
        qrencode -t UTF8 < ~/ssr.conf
        echo
        echo -e "SS QR CODE: "
        echo
        qrencode -t UTF8 < ~/ss.conf
        echo
        rm -r ~/ssr.conf
        rm -r ~/ss.conf
        echo
...
Debian_apt(){
	apt-get update
	apt-get install -y vim unzip cron git net-tools qrencode iftop curl vnstat
}
# 下载 ShadowsocksR
Download_SSR(){
	cd "/usr/local"
